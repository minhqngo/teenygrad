...
def fetch_mnist(base):
    parse = lambda file: np.frombuffer(gzip.open(file).read(), dtype=np.uint8).copy()
    X_train = parse(base + "/train-images-idx3-ubyte.gz")[0x10:].reshape((-1, 28 * 28)).astype(np.float32)
    Y_train = parse(base + "/train-labels-idx1-ubyte.gz")[8:]
    X_test = parse(base + "/t10k-images-idx3-ubyte.gz")[0x10:].reshape((-1, 28 * 28)).astype(np.float32)
    Y_test = parse(base + "/t10k-labels-idx1-ubyte.gz")[8:]
    return X_train, Y_train, X_test, Y_test


X_train, Y_train, X_test, Y_test = fetch_mnist(base="/home/minh/datasets/MNIST-gz")
