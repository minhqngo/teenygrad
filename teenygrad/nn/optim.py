...
class LAMB(Optimizer):
    ...
    def step(self) -> None:
        ...
        for i, t in enumerate(self.params):
            ...
            m_update = self.b1 * self.m[i] + (1.0 - self.b1) * g
            self.m[i].assign(m_update).realize()

            # Update second moment (mean of squared gradients)
            v_update = self.b2 * self.v[i] + (1.0 - self.b2) * (g * g)
            self.v[i].assign(v_update).realize()
